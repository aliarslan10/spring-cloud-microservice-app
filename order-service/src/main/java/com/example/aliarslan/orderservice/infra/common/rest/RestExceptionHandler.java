package com.example.aliarslan.orderservice.infra.common.rest;
...
public class RestExceptionHandler  extends BaseController {
    ...
    public Response<ErrorResponse> handleOrderBusinessException(OrderBusinessException ex, Locale locale) {
        return createErrorResponseFromMessageSource(ex.getKey(), locale, ex.getArgs());
    }

    @ExceptionHandler(OrderNotFoundException.class)
    @ResponseStatus(HttpStatus.UNPROCESSABLE_ENTITY)
    public Response<ErrorResponse> handleOrderApiDataNotFoundException(OrderNotFoundException ex, Locale locale) {
        return createErrorResponseFromMessageSource(ex.getKey(), locale, ex.getArgs());
    }
